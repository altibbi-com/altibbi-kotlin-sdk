...
class CreateUserActivity : AppCompatActivity() {
    ...
    private fun submitCreate() {
        ...
        ApiService.createUser(user, object : ApiCallback<User> {
            override fun onSuccess(response: User) {
                runOnUiThread {
                    setSubmitting(false)
                    showStatus("User created — ID: ${response.id}, Name: ${response.name}", false)
                    clearForm()
                }
            }
            override fun onFailure(error: String?) {
                runOnUiThread {
                    setSubmitting(false)
                    showStatus(error ?: "Failed to create user.", true)
                }
            }
    ...
    private fun setSubmitting(loading: Boolean) {
        ...
        btn.text = if (loading) "Creating…" else "Create User"
    }
